 ...
 * 黑市上架记录界面
 */
public class BlackLaunchRecord extends LaunchRecordUI {
    // 单例实例
    private static BlackLaunchRecord instance;
    
    /**
     * 私有构造函数
     */
    private BlackLaunchRecord() {
        super("黑市上架记录", 54, new BlackMarketStrategy());
    }
    
    /**
     * 获取实例
     * @return BlackLaunchRecord实例
     */
    public static BlackLaunchRecord getInstance() {
        if (instance == null) {
            instance = new BlackLaunchRecord();
        }
        return instance;
    }
    ...
    protected void refreshItems(Player player, int page) {
        // 获取黑市数据
        BlackMarketFile blackMarketFile = MMGlobalExchanges.instance.getBlackMarketFile();
        Map<String, Object> marketData = blackMarketFile.getData();
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            ItemStack item = blackMarketFile.getItemStack(id);
