 ...
 * 市场购买记录界面
 */
public class PurchaseRecord extends PurchaseRecordUI {
    // 单例实例
    private static PurchaseRecord instance;
    
    /**
     * 私有构造函数
     */
    private PurchaseRecord() {
        super("购买记录", 54, new NormalMarketStrategy());
    }
    
    /**
     * 获取实例
     * @return PurchaseRecord实例
     */
    public static PurchaseRecord getInstance() {
        if (instance == null) {
            instance = new PurchaseRecord();
        }
        return instance;
    }
